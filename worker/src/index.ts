...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (request.method !== "POST" || new URL(request.url).pathname !== "/gemini") return json({ error: "未找到服务" }, 404, origin, env);
    ...
    if (!apiKey || !prompt) return json({ error: "需要 Gemini API Key 与创作内容" }, 400, origin, env);
